  ...
  <section class="cart_module">
    <section class="cart_button">
      ...
      <transition name="showReduce">
        <span @click="removeOutCart(foods.id, foods.name, foods.rebate_price)" v-if="foodNum">
          <svg>
      ...
      <svg class="add_icon" @touchstart="addToCart( foods.id, foods.name, foods.rebate_price, $event)">
...
export default {
  ...
  computed: {
    ...
    foodNum: function () {
      ...
      // Api.log(`当前购物车的事物数据：${JSON.stringify(foods)}`)
      // Api.log(`当前shopCart数据：${JSON.stringify(this.shopCart)}`)
      let curFood = this.shopCart[this.foods.id]
      if (curFood) {
        let num = curFood && curFood.num;
        return num;
      } else {
        return 0;
      }
  ...
  methods: {
    ...
    removeOutCart(food_id, name, price) {
      if (this.foodNum > 0) {
        this.REDUCE_CART({ shopid: this.shopId, food_id, name, price });
      }
    },
    //加入购物车，计算按钮位置。
    addToCart(food_id, name, price, event) {
      this.ADD_CART({ shopid: this.shopId, food_id, name, price });
